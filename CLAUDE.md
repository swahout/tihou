...
- `RaceMarkTable` の結果は未確定時（レース前）は空になる
- `OddsTanFuku` の体重セル `[7]` は "480(+4)" 形式または "(+4)" 形式で場によって異なる
  → 実物を確認してからパースすること
- PC版のみ利用可（SPサイトは別URL、keiba.go.jpはJSなしで取得可能）

### OddsTanFuku の列順（実物で必ず確認）

```
[0]=枠, [1]=馬番, [2]=馬名, [3]=単勝オッズ, [4]=複勝下, [5]=複勝上,
[6]=性齢, [7]=体重(変化), [8]=負担重量, [9]=騎手, [11]=調教師
```
