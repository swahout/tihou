...
- **本番モデルは v10**（v6 + 過去人気ラグ4特徴, 37特徴量, **当日人気は不使用・履歴のみ**）。**v9の当日人気ブレンドと併用可**。`data/kawasaki_best_params.json` は v6ハイパラのまま（v10での再チューニングは未実施＝任意の次タスク）。
- **v10の狙い**: 当日人気は変動するので使いたくない、という方針に対し「その馬の**過去**人気＝確立した市場評価」を特徴量化。当日オッズ無しで model単独 top1 57.1→59.9% / top5 69.2→70.6%（LOYO）。
- **市場の再現性検証(`analyze_market.py`)**: 特徴量からpopularityを予測しても top3 48%(=モデル同等)止まり。当日人気の残差価値+9.9ppは現特徴量と**直交する当日情報**(パドック/調教/資金流)。→ **当日人気を完全に置換して市場58%に並ぶのは不可**。持続的実力(履歴)は取れるが当日固有の合意は取れない。
- 当日オッズがある場面では **v9ブレンド(w_model=0.2)が最良**: top5 78.3% / top3 58.3% / top1 74.2%（市場支配的, モデル寄与は僅少）。
...
2. ~~過去人気ラグの統合~~ → **完了(v10)。当日人気なしで model単独 +1.4/+2.8pp（§0）。**
3. **（任意）v10 の再チューニング**: lag_pop_* 追加で37特徴量になったが Optuna は未再実行（現状 v6ハイパラ流用で既に改善済み）。`python kawasaki_predict.py --tune --trials 150`（study `kawasaki_multiyr_8R_top5_v10`、数時間）。ブレンド時のモデル寄与は僅少なので優先度は低い。
4. **v10+v9 を運用確定**し「軸1頭+手広く流す」で実運用（top1 74%）。**発走直前にオッズ再収集**（前日だと当日ブレンド無し＝v10モデル単独 top1 60%にフォールバック）。
5. **期待値ベース馬券へ方向転換の検討**: モデルは市場をほぼ超えない（§analyze_market）。的中率より「市場と乖離した妙味馬」を拾う回収率設計が実益大の可能性。`OddsTanFuku`複勝オッズと組合せ可。
